#!/usr/bin/env python3
"""
CSS10データセットをLJSpeechフォーマットに変換するスクリプト（重複回避版）

使用方法:
    python convert_to_ljspeech_no_duplicates.py
...
def find_wav_file_priority_lang(base_path, file_path_from_transcript):
    """WAVファイルの実際の場所を探す（言語ディレクトリ優先）"""
    lang_code = get_language_from_archive(base_path.name)
    
    # 言語ディレクトリを優先的に確認
    if lang_code != 'unknown':
        lang_subfolder_path = base_path / lang_code / file_path_from_transcript
        if lang_subfolder_path.exists():
            return lang_subfolder_path
    
    # 言語ディレクトリにない場合はルートレベルを確認
    root_path = base_path / file_path_from_transcript
    if root_path.exists():
        return root_path
    
    return None

def process_archive_no_duplicates(archive_path, output_wavs_dir, metadata_rows):
    """1つのアーカイブを処理する（重複回避）"""
    ...
    for line in tqdm(lines, desc=f"{archive_name}"):
        ...
        # WAVファイルを探す（言語ディレクトリ優先）
        wav_file = find_wav_file_priority_lang(archive_path, file_path_from_transcript)
    ...
    print(f"  - 言語ディレクトリ使用: {lang_dir_used}ファイル")
    print(f"  - ルートディレクトリ使用: {root_dir_used}ファイル")
...
def main():
    """メイン処理"""
    data_dir = Path('data')
    output_dir = Path('raw_no_duplicates')
    ...
    # 各アーカイブを処理
    for archive_dir in archive_dirs:
        metadata_rows = process_archive_no_duplicates(archive_dir, output_wavs_dir, metadata_rows)
    ...
    print(f"重複回避により、元のデータセットより約半分のサイズになっているはずです")
