#!/usr/bin/env python3
"""
scipy.io.wavfileを使用してWAVファイルの音声長を詳細に分析するスクリプト
IEEE浮動小数点フォーマット対応
"""
...
def get_audio_duration(wav_file):
    """scipy.io.wavfileを使用してWAVファイルの音声長を取得（秒）"""
    try:
        # scipy.io.wavfileで音声ファイルを読み込み
...
def analyze_durations():
    """音声長を分析"""
    print("=== scipy.io.wavfileを使用したWAVファイル音声長分析 ===\n")
...
def create_duration_histogram(durations, lang_durations):
    ...
    plt.tight_layout()
    plt.savefig('audio_duration_analysis_scipy.png', dpi=300, bbox_inches='tight')
    print(f"\n分布図を 'audio_duration_analysis_scipy.png' に保存しました")
