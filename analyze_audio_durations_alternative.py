...
def get_audio_duration_manual(wav_file):
    """WAVファイルの音声長を手動で取得（秒）"""
    try:
...
def get_audio_duration_simple(wav_file):
    """簡易的な音声長取得（ファイルサイズから推定）"""
    try:
        file_size = wav_file.stat().st_size
        # WAVヘッダー（44バイト）を除いたデータサイズ
        data_size = file_size - 44
        # 一般的なWAV設定（16bit, 22050Hz, モノラル）で推定
        sample_rate = 22050
        bytes_per_sample = 2
...
def analyze_durations():
    ...
    # 中国語ファイルを除外
    filtered_wav_files = [f for f in wav_files if not f.stem.startswith('zh_')]
    
    print(f"分析対象: {len(filtered_wav_files)}ファイル（中国語{len(wav_files) - len(filtered_wav_files)}ファイルを除外）")
    print("音声長を計算中...")
    ...
    for i, wav_file in enumerate(filtered_wav_files):
        if i % 5000 == 0:
            print(f"  進捗: {i}/{len(filtered_wav_files)}")
        
        # まず手動解析を試す
        duration = get_audio_duration_manual(wav_file)
        if duration is None:
            # 失敗した場合は簡易推定を試す
            duration = get_audio_duration_simple(wav_file)
    ...
    print(f"\n極値ファイルを特定中...")
    for wav_file in filtered_wav_files:
        duration = get_audio_duration_manual(wav_file)
        if duration is None:
            duration = get_audio_duration_simple(wav_file)
